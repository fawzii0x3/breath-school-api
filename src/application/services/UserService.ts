...
import { UserCreateRequest, UserResponse } from '../../domain/entities/User';
...
export class UserService implements IUserService {
  ...
  async createUser(userData: UserCreateRequest & { _id?: string }): Promise<{ success: boolean; info: string; data?: UserResponse }> {
  ...
  async deleteUser(userId: string): Promise<{ success: boolean; info: string }> {
    ...
    if (result.success) {
      try {
        ...
        const tagsResult = await this.systemeService.getUserTags(userId);
